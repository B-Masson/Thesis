...
desc = "Going back to norm per, as it was more consistent. Still attempting to resolve OOM issue."
print(desc)
import os
...
curated = False
trimming = True
modelname = "ADModel_K_v3.3" #Next in line: ADMODEL_NEO_v1.3
logname = "K_V3.3" #Neo_V1.3
if not testing_mode:
...
for train_index, val_index in skf.split(x, y):
    ...
    def make_unique(path, file_name):
        if os.path.isfile(path+file_name):
            #print("I have determined that", file_name, "already exists.")
            expand = 1
            while True:
                new_file_name = path +str(expand) + "_" + file_name
    ...
    if plotting:
        try:
            ...
            # Plot stuff
            plotname = plotname + "fold" + str(fold)
            ...
            name = make_unique(plotpath, name)
            plt.savefig(name)
            plt.clf()
