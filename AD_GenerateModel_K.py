...
# Richard Masson
# Info: Most advanced version of the code so far. K-fold & forced diversity are new additions.
# Last use in 2021: November 18th
print("IMPLEMENTATION: STANDARD")
print("CURRENT TEST: Trying to implement a k-fold validation process.")
import os
...
if testing_mode:
    ...
    if force_diversity:
        x, x_test, y, y_test = train_test_split(x_arr, y_arr, stratify=y_arr, test_size=0.2)
    else:
        x, x_test, y, y_test = train_test_split(x_arr, y_arr, test_size=0.2)
...
if testing_mode:
    ...
    epochs = 30
    batches = 10 # Going to need to fiddle with this over time (balance time save vs. running out of memory)
'''
...
# Ensure that the test data is prepped and ready.
...
print("Starting cross-fold validation process...")
fold = 0
for train_index, val_index in skf.split(x, y):
    ...
    print("Example of y-data we are working with:", y_train[0:2])
    ...
    checkpointname = "weights/weight_history_" +str(fold) + ".h5"
    if testing_mode:
        checkpointname = "test_weights/weight_history_testing_" +str(fold) + ".h5"
    mc = ModelCheckpoint(checkpointname, monitor='val_loss', mode='min', verbose=0, save_best_only=False) #Maybe change to true so we can more easily access the "best" epoch
    if testing_mode:
        log_dir = "/scratch/mssric004/test_logs/fit/" + str(fold) + "_" +datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    else:
        if logname != "na":
            log_dir = "/scratch/mssric004/logs/fit/" + logname + "_" + str(fold) + "_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        else:
            log_dir = "/scratch/mssric004/logs/fit/" + str(fold) + "_" +datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    tb = TensorBoard(log_dir=log_dir, histogram_freq=1)

    # Run the model
    #print("Fitting model...")
    history = model.fit(train_set, validation_data=validation_set, batch_size=batches, epochs=epochs, verbose=0, callbacks=[tb, es], class_weight=class_weight)
    # NOTE: ADD CHECKPOINTS BACK IN at some point
    modelname = "ADModel_Shin/ADModel_Shin_" + str(fold)
    if testing_mode:
    ...
    print("Ran for ", actual_epochs, "/", epochs, " epochs.\nParameters saved to", modelname)
